    ...
    Public Function ObtenerListadoReporteCatalogo() As DataTable
        ...
        Try
            ...
            If (Me.EId > 0) Then
                condicion &= " AND Id=@id"
            End If
            comando.CommandText = String.Format("SELECT Id, Nombre FROM {0}SubFamilias WHERE 0=0 {1} ORDER BY IdAlmacen, IdFamilia, Id ASC", ALMLogicaPedidos.Programas.prefijoBaseDatosAlmacen, condicion)
    ...
    Public Function ObtenerListado() As List(Of SubFamilias)
        ...
        Try
            ...
            If (Me.EId > 0) Then
                condicion &= " AND Id=@id"
            End If
            comando.CommandText = String.Format("SELECT IdAlmacen, IdFamilia, Id, Nombre FROM {0}SubFamilias WHERE 0=0 {1}", ALMLogicaPedidos.Programas.prefijoBaseDatosAlmacen, condicion)
            comando.Parameters.AddWithValue("@idAlmacen", Me.EIdAlmacen)
            comando.Parameters.AddWithValue("@idFamilia", Me.EIdFamilia)
            comando.Parameters.AddWithValue("@id", Me.EId)
            BaseDatos.conexionCatalogo.Open()
