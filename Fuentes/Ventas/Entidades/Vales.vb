...
Public Class Vales
    ...
    Public Property EIdVenta() As Integer
        ...
        End Set
    End Property
    Public Property ETotalVenta() As Double
        Get
            Return totalVenta
        End Get
        Set(value As Double)
    ...
    Public Sub Guardar()
        ...
        Try
            ...
            comando.CommandText = String.Format("INSERT INTO Vales (IdOrigen, Id, IdCliente, Fecha, Total, FechaVencimiento, EstaUtilizado, IdVenta, TotalVenta) VALUES (@idOrigen, @id, @idCliente, @fecha, @total, @fechaVencimiento, @estaUtilizado, @idVenta, @totalVenta)")
            ...
            comando.Parameters.AddWithValue("@totalVenta", Me.ETotalVenta)
    ...
    Public Function ObtenerListadoImpresionVales() As DataTable
        ...
        Try
            ...
            End If
            'If (Me.EIdVenta > 0) Then
            '    condicion &= " AND V.IdVenta=@idVenta"
            'End If
            comando.CommandText = String.Format("SELECT V.IdOrigen, V.Id, VD.IdAlmacen, A.Nombre AS NombreAlmacen, VD.IdFamilia, F.Nombre AS NombreFamilia, VD.IdSubFamilia, SF.Nombre AS NombreSubFamilia, VD.IdArticulo, A2.Nombre AS NombreArticulo, UM.Nombre AS NombreUnidad, A2.Codigo, A2.Pagina, A2.Color, A2.Talla, A2.Modelo, A2.CodigoInternet, V.IdCliente, C.Nombre AS NombreCliente, V.Fecha, VD.Cantidad, VD.Precio, V.Total, V.FechaVencimiento " & _
            " FROM Vales AS V " & _
    ...
    Public Sub EditarParaEliminar()
        ...
        Try
            ...
            Dim condicion As String = String.Empty 
            If (Me.EIdVenta > 0) Then
                condicion &= " AND IdVenta=@idVenta"
            End If
            comando.CommandText = String.Format("UPDATE VALES SET IdVenta=0, EstaUtilizado='FALSE' WHERE 0=0 {0}", condicion)
            comando.Parameters.AddWithValue("@idVenta", Me.EIdVenta)
            BaseDatos.conexionAlmacen.Open()
            comando.ExecuteNonQuery()
            BaseDatos.conexionAlmacen.Close()
    ...
    Public Sub Editar()
        ...
        Try
            ...
            If (Me.EId > 0) Then
                condicion &= " AND Id=@id"
            End If
            comando.CommandText = String.Format("UPDATE VALES SET IdVenta=@idVenta, EstaUtilizado='TRUE', TotalVenta=@totalVenta WHERE 0=0 {0}", condicion)
            comando.Parameters.AddWithValue("@idOrigen", Me.EIdOrigen)
            comando.Parameters.AddWithValue("@id", Me.EId)
            comando.Parameters.AddWithValue("@idVenta", Me.EIdVenta)
            comando.Parameters.AddWithValue("@totalVenta", Me.ETotalVenta)
    ...
    Public Function ObtenerListadoDetallado() As DataTable
        ...
        Try
            ...
            If (Me.EIdVenta > 0) Then
                condicion &= " AND IdVenta=@idVenta"
            End If
            comando.CommandText = String.Format("SELECT IdOrigen, Id, ISNULL(SUM(Total), 0) AS Total, ISNULL(SUM(TotalVenta), 0) AS TotalVenta " & _
            " FROM Vales " & _
            " WHERE 0=0 {1} " & _
            " GROUP BY IdOrigen, Id", ALMLogicaVentas.Programas.bdCatalogo & ".dbo." & ALMLogicaVentas.Programas.prefijoBaseDatosAlmacen, condicion)
            comando.Parameters.AddWithValue("@idVenta", Me.EIdVenta)
            BaseDatos.conexionAlmacen.Open()
            Dim lectorDatos As SqlDataReader
            lectorDatos = comando.ExecuteReader()
    ...
    Public Function ObtenerListado() As DataTable
        ...
        Try
            ...
            End If
            comando.CommandText = String.Format("SELECT IdOrigen, Id, Fecha, ISNULL(SUM(Total), 0) AS Total " & _
            " FROM Vales " & _
            " WHERE EstaUtilizado='FALSE' {1}" & _
            " GROUP BY IdOrigen, Id, Fecha ", ALMLogicaVentas.Programas.bdCatalogo & ".dbo." & ALMLogicaVentas.Programas.prefijoBaseDatosAlmacen, condicion)
            comando.Parameters.AddWithValue("@idOrigen", Me.EIdOrigen)
            comando.Parameters.AddWithValue("@id", Me.EId)
            comando.Parameters.AddWithValue("@idCliente", Me.EIdCliente)
            BaseDatos.conexionAlmacen.Open()
